...
def run(args, device, data, tic):
	...
	for epoch in range(args.num_epochs):
		...
		if epoch % args.eval_every==0 and epoch!=0:
			eval_acc = evaluate(evaluate_dataloader, model_engine, val_g, val_nfeat, val_labels, val_nid, args, timers,
				device)
			print('Eval Acc {:.4f}'.format(eval_acc))
			test_acc = evaluate(test_dataloader, model_engine, test_g, test_nfeat, test_labels, test_nid, args, timers,
				device)
